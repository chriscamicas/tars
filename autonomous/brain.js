class Brain {
  ...
  crossover(otherBrain) {
    ...
    tf.tidy(() => {
      ...
      for (let i = 0; i < myWeights.length; i++) {
        ...
        // myValues = this.halfCrossover(myValues, otherValues)
        // myValues = this.meanCrossover(myValues, otherValues)
  ...
  halfCrossover(myValues, otherValues) {
    let mid = Math.floor(myValues.length / 2)
    for (let j = mid; j < myValues.length; j++) {
      myValues[j] = otherValues[j]
    }
    return myValues
  }
  meanCrossover(myValues, otherValues) {
    for (let j = 0; j < myValues.length; j++) {
      myValues[j] = (myValues[j] + otherValues[j]) / 2
    }
    return myValues
  }
}
